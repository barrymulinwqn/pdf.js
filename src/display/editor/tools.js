...
class AnnotationEditorUIManager {
  ...
  saveSelectionAsJson(methodOfCreation = "") {
    try {
      ...
      if (!pageElement) {
        warn("Could not find page element");
        return;
      }
      ...
      if (!textLayer) {
        warn("Could not find text layer");
        return;
      }
      ...
      if (!selectedText) {
        warn("No text content in selection");
        return;
      }
      ...
      // Get the annotation editor layer to access page view
      const annotationEditorLayer = this.#allLayers.get(pageNumber);
      if (!annotationEditorLayer) {
        warn("Could not find annotation editor layer");
        return;
      }

      // Get page dimensions to convert coordinates
      const pageDiv = annotationEditorLayer.div;
      ...
      // Convert to PDF coordinates (approximation)
      // In a proper implementation, you would need the actual page viewport
      const scale = pageDivRect.width / pageDiv.offsetWidth || 1;
      const location = [
        Math.round(x / scale),
        Math.round(y / scale),
      ...
      try {
        ...
        warn(`Error dispatching custom event: ${eventError}`);
      }
      ...
      if (typeof window !== "undefined" && window.parent !== window) {
        try {
          ...
          console.log("Selection data sent to parent window for postMessage:", data);

        } catch (postError) {
          warn(`Error posting message to parent: ${postError}`);
        }
      }

      console.log("Selection data sent to parent window:", data);

      // Hide the floating toolbar after saving
      this.#floatingToolbar?.hide();
    } catch (error) {
      warn(`Error saving selection as JSON: ${error}`);
    }
  }
